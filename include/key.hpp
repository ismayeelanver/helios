...
inline void grab_key(xcb_connection_t *conn, uint32_t keysym,
                     uint32_t modifiers, xcb_window_t window) {
  ...
  xcb_void_cookie_t status =
      xcb_grab_key(conn, 1, window, modifiers, *keycode, XCB_GRAB_MODE_ASYNC,
                   XCB_GRAB_MODE_ASYNC);
